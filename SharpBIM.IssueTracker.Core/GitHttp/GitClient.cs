...
namespace SharpBIM.IssueTracker.Core.GitHttp
{
    public abstract class GitClient : SharpBIMHTTP
    {
        ...
        protected override string GetEndPoint(params object[] values)
        {
            ...
            List<object> listValues = new List<object>(values);
            if (repoModel != null)
            {
                listValues = values.Skip(1).ToList();
...
#if WINDOWS
        internal IssueTrackerUser User => IssueTrackerGlobals.AppGlobals.User;
#else
        ...
        protected override StringContent GetStringContent(object requestBody)
        {
            if (requestBody != null)
            {
                var content = new StringContent(JsonSerializer.Serialize(requestBody, GetJsonOptions(null)), Encoding.UTF8, MediaTypesStatics.VNDGITHUBJSON);
                return content;
            }
            return null;
        }
        ...
        protected override async Task<IServiceReport<string>> PATCH(string url, object requestBody)
        {
            ...
            {
                {
                    Content = new StringContent(JsonSerializer.Serialize(requestBody, GetPostOptions()), Encoding.UTF8, MediaTypesStatics.FULLJSON)
                }
                ;
                LoadHeaders(request);
                var response = await httpClient.SendAsync(request);
