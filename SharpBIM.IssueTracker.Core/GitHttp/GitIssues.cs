...
namespace SharpBIM.IssueTracker.Core.GitHttp
{
    public class GitIssues : GitClient
    {
        ...
        protected override void AddHeaders(HttpRequestMessage request)
        {
            ...
            //  request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(MediaTypesStatics.TXTJSON)); //textOnly
            request.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(MediaTypesStatics.FULLJSON)); //txt, html, markdown
        }
        ...
        public async Task<IServiceReport<IssueModel>> CreateIssue(RepoModel repoModel, IssueModel issue)
        {
            ...
            //  var content = new StringContent(issue.JSerialize(), Encoding.UTF8, MediaTypesStatics.VNDGITHUBJSON);
            var paybody = new
            {
                issue.body,
                title = issue.Title,
